...
try {
    // Get the latest fire_data record for the user (for routing purposes)
    // This endpoint gets the latest fire_data regardless of status
    $sql = "
    SELECT 
        id,
        status,
    ...
    if (!$deviceWithinRadius) {
        echo json_encode([
            ...
            'message' => 'Device must be inside a building radius area to calculate route to fire station. Please move the device within a building\'s radius (100 meters) and try again.',
            'device_id' => $device_id,
    ...
    // Return the fire data with coordinates (device is within building radius)
    $response = [
        'success' => true,
        'data' => [
            'id' => $fireData['id'],
    ...
    error_log("Database error in get_latest_fire_data_for_route.php: " . $e->getMessage());
    echo json_encode([
        'success' => false, 
