<?php
/**
 * Device Smoke GPS API - Secure Version
 * Handles GPS and sensor data from IoT fire detection devices
 */
...
class Database {
    ...
    private static function getCredentials() {
        ...
        $coreConfigPath = __DIR__ . '/../core/config/config.php';
    ...
    public static function getConnection() {
        ...
        if ($conn === null) {
            try {
                ...
                if (empty($creds['dbname']) || empty($creds['username'])) {
                    error_log("CRITICAL: Database configuration incomplete in smoke_gps.php");
                    return null;
                }
...
class SmokeAPI {
    ...
    public function __construct() {
        ...
        // Check if it's JSON (GPS data) or form data (sensor data)
        if (!empty($input)) {
            ...
            if (strpos($contentType, 'application/json') !== false) {
                ...
                $this->processGpsData($data);
            } else {
                // Form data (sensor data)
                parse_str($input, $data);
                $this->processSensorData($data);
            }
        } else {
            // GET parameters (sensor data)
            $data = $_GET;
            $this->processSensorData($data);
        }
    }
    ...
    private function processSensorData($data) {
        ...
        $this->reason = isset($data['reason']) ? $data['reason'] : 'regular';
    }
    
    private function processGpsData($data) {
        ...
        $this->altitude = isset($data['altitude']) && $data['altitude'] !== 'null' ? floatval($data['altitude']) : null;
        $this->ph_time = isset($data['ph_time']) ? $data['ph_time'] : null;
        $this->satellites = isset($data['satellites']) && $data['satellites'] !== 'null' ? intval($data['satellites']) : null;
        $this->hdop = isset($data['hdop']) && $data['hdop'] !== 'null' ? floatval($data['hdop']) : null;
        $this->device_id = 1; // Default device for GPS
    }
    
    public function processRequest() {
        try {
            // Check if this is GPS data or sensor data
            if ($this->isGpsData()) {
                return $this->processGpsRequest();
            } else {
                return $this->processSensorRequest();
            }
        } catch (Exception $e) {
            $this->handleError($e);
        }
    ...
    private function isGpsData() {
        return $this->latitude !== null && $this->longitude !== null;
    }
    
    private function processSensorRequest() {
        $this->validateDevice();
    ...
    private function processGpsRequest() {
        $gps_insertion = $this->insertGpsData();
        ...
        error_log("GPS Data received: Lat={$this->latitude}, Lon={$this->longitude}, Time={$this->ph_time}");
    ...
    private function insertGpsData() {
        ...
        $ph_time = $this->ph_time ?: date('Y-m-d H:i:s');
        
        // Handle null values properly
        $latitude = $this->latitude !== null ? $this->latitude : 0.0;
        $longitude = $this->longitude !== null ? $this->longitude : 0.0;
        $altitude = $this->altitude !== null ? $this->altitude : 0.0;
        $satellites = $this->satellites !== null ? $this->satellites : 0;
        $hdop = $this->hdop !== null ? $this->hdop : 0.0;
        
        $stmt = $conn->prepare("INSERT INTO gps_data (latitude, longitude, altitude, ph_time, satellites, hdop) VALUES (?, ?, ?, ?, ?, ?)");
        
        if (!$stmt) {
            error_log("Prepare failed for GPS: " . $conn->error);
            return 'failed';
        }
        
        // Use proper data types for bind_param
        $stmt->bind_param("dddssd", 
            $latitude, 
            $longitude, 
            $altitude, 
            $ph_time,
        ...
        if (!$success) {
            error_log("Error inserting GPS data: " . $stmt->error);
            return 'failed';
        }
    ...
    private function logData() {
        ...
        error_log("Device {$this->device_id} data: Smoke={$this->value} (detected={$this->detected}), Flame={$this->flame_detected}, Temp={$temp_str}°C, Humidity={$humidity_str}%, Heat Index={$heat_index_str}°C, Reason={$this->reason}");
    }
}
