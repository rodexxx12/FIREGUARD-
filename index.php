...
<head>
    ...
    <style>
        * {
        ...
        .forgot-container .login-btn:hover,
    ...
    <nav>
        <div class="logo">
            <div class="logo-icon-wrapper">
                <img src="fire/images/logo1.png" alt="FireGuard Logo" class="logo-img">
            </div>
        ...
        <div class="inline-login-panel" id="inlineLoginPanel" aria-hidden="true">
            ...
            <form class="inline-login-form" id="inlineLoginForm" method="post" autocomplete="off">
                ...
                <div class="captcha-container">
                    ...
                    $recaptchaConfigPath = __DIR__ . '/login/functions/recaptcha_config.php';
    ...
    <script>
        ...
        document.querySelectorAll('a[href^="#"]').forEach(anchor => {
            anchor.addEventListener('click', function (e) {
                ...
                // Ignore bare "#" links which are not actual section targets
                if (!href || href.trim() === '#') {
                    return;
                }
        ...
        if (forgotContainer && !forgotContainer.hasAttribute('aria-hidden')) {
            forgotContainer.setAttribute('aria-hidden', 'true');
        }

        if (resetContainer && !resetContainer.hasAttribute('aria-hidden')) {
        ...
        function safelyHidePanel(panel) {
            ...
            // Immediately blur any focused elements synchronously
            const focusedElement = panel.querySelector(':focus');
            const activeElement = document.activeElement;
            
            // Blur any focused elements immediately (synchronously)
            ...
            // Use a small timeout to ensure blur has taken effect before setting aria-hidden
            // This prevents the browser warning about aria-hidden on focused elements
            setTimeout(() => {
                // Final check - if something still has focus, blur it
                const stillFocused = panel.querySelector(':focus');
                if (stillFocused && typeof stillFocused.blur === 'function') {
                    stillFocused.blur();
                }
                
                // Now safely set aria-hidden
        ...
        function openInlineLogin() {
            ...
            // Clear validation feedback when opening login panel
            const usernameGroup = document.getElementById('usernameGroup');
            const usernameFeedback = document.getElementById('usernameFeedback');
            const passwordGroup = document.getElementById('passwordGroup');
            const passwordFeedback = document.getElementById('passwordFeedback');
            
            if (usernameGroup && usernameFeedback) {
                hideValidationFeedback(usernameGroup, usernameFeedback);
            }
            if (passwordGroup && passwordFeedback) {
        ...
        function openForgotModal() {
            ...
            forgotContainer.setAttribute('aria-hidden', 'false');
            setTimeout(() => {
                if (forgotEmailInput) {
        ...
        function setForgotButtonState(isLoading) {
            if (!forgotButton || !forgotButtonText) return;
            forgotButton.disabled = isLoading;
            forgotButtonText.textContent = isLoading ? 'Sending...' : 'SEND RESET LINK';
        }
        ...
        if (forgotPasswordLink && forgotContainer) {
        ...
        function showValidationFeedback(group, feedbackElement, type, message) {
            if (!group || !feedbackElement) return;
            
            // Remove existing classes
            group.classList.remove('has-error', 'has-success');
            feedbackElement.classList.remove('error', 'success', 'info', 'show');
            ...
            if (type === 'error') {
                ...
                feedbackElement.classList.add('info', 'show');
                feedbackElement.textContent = message;
            }
        }

        function hideValidationFeedback(group, feedbackElement) {
            if (!group || !feedbackElement) return;
            group.classList.remove('has-error', 'has-success');
            feedbackElement.classList.remove('error', 'success', 'info', 'show');
        ...
        if (inlineLoginForm) {
            inlineLoginForm.addEventListener('submit', function(e) {
                e.preventDefault();
                const username = document.getElementById('inlineLoginUsername').value;
                ...
                if (!username || !password) {
                    ...
                    if (!username) {
                        showValidationFeedback(usernameGroup, usernameFeedback, 'error', 'Username is required');
                    }
                    if (!password) {
                        showValidationFeedback(passwordGroup, passwordFeedback, 'error', 'Password is required');
                    }
                    return;
                }
                ...
                if (!captchaResponse) {
                    if (captchaError) {
                        ...
                        captchaError.classList.add('show', 'error');
                    }
                    return;
                } else {
                    if (captchaError) {
                        captchaError.classList.remove('show', 'error');
                        captchaError.textContent = '';
                    }
                }
                
                // Blur the button to prevent aria-hidden warning
                if (loginButton && document.activeElement === loginButton) {
                    loginButton.blur();
                }
                    ...
                    if (!response.ok) {
                        ...
                        return response.text().then(text => {
                            ...
                            try {
                                ...
                                // If not JSON, use the text (limited length)
                                if (text) {
                ...
                .then(data => {
                    // Ensure button is blurred before any redirect or panel hiding
                    if (loginButton && document.activeElement === loginButton) {
                        loginButton.blur();
                    }
                    ...
                    if (data.success) {
                        ...
                        // Small delay to ensure blur completes before redirect
                        setTimeout(() => {
                            // Redirect on success using replace to prevent back navigation
                            if (data.redirect) {
        ...
        if (forgotPasswordForm) {
            forgotPasswordForm.addEventListener('submit', function(e) {
                ...
                formData.append('email', forgotEmailInput.value.trim());
                ...
                .then(async response => {
                    ...
                    if (!response.ok) {
                        ...
                        try {
                            ...
                            if (rawText) {
                                message += ': ' + rawText.substring(0, 150);
                            }
                        }
                    ...
                    try {
                        ...
                        console.error('Forgot password JSON parse error:', parseError, rawText);
        ...
        // Note: Forward navigation prevention for dashboard pages is handled by
        // the standalone script: login/js/prevent-forward-navigation.js
        // Include that script in your dashboard pages to enable forward button blocking
        // Example: <script src="../login/js/prevent-forward-navigation.js"></script>
    </script>
</body>
</html>
