...
$firefighterName = 'Admin';

if (!empty($_SESSION['firefighter_name'])) {
    $firefighterName = $_SESSION['firefighter_name'];
} elseif (!empty($_SESSION['firefighter_id'])) {
    $firefighterId = (int) $_SESSION['firefighter_id'];
    $firefighter = fetchSingle("SELECT name FROM firefighters WHERE id = ?", [$firefighterId]);

    if ($firefighter && !empty($firefighter['name'])) {
        $firefighterName = $firefighter['name'];
        $_SESSION['firefighter_name'] = $firefighterName;
    } elseif (!empty($_SESSION['username'])) {
        $firefighterName = $_SESSION['username'];
    }
} elseif (!empty($_SESSION['username'])) {
    $firefighterName = $_SESSION['username'];
}
...
<div class="clearfix"></div>
  <div class="profile clearfix">
    ...
    <div class="profile_info">
      <span>Welcome,</span>
      <h2><?php echo htmlspecialchars($firefighterName); ?></h2>
    </div>
  </div>
<br />
