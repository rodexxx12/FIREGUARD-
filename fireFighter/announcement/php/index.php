...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'])) {
    ...
    try {
        ...
        if (isset($_SESSION['firefighter_id'])) {
            ...
            // Get all announcements including superadmin announcements
            $announcements = getAllFirefighterAnnouncements($firefighterId, true, true);
        } else {
            // Fallback to regular user announcements
...
<head>
    ...
    <style>
        ...
        /* Superadmin announcement styling */
        .announcement-item.superadmin {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function displayLatestAnnouncement(announcement) {
                ...
                // Set priority badge with announcement type indicator
                ...
                const announcementType = announcement.announcement_type === 'superadmin' ? 'Superadmin' : 'Regular';
                const typeIcon = announcement.announcement_type === 'superadmin' ? '👑' : '📢';
                latestPriorityBadge.innerHTML = `
                    <span class="${priorityClass}">${priorityText} Priority</span>
                    <div class="mt-2">
                        <small class="badge bg-info">${typeIcon} ${announcementType} Announcement</small>
                    </div>
                `;
            ...
            function displayPreviousAnnouncements(announcements) {
                ...
                announcements.forEach(announcement => {
                    ...
                    const baseClass = `list-group-item announcement-item priority-${announcement.priority}`;
                    const superadminClass = announcement.announcement_type === 'superadmin' ? ' superadmin' : '';
                    item.className = baseClass + superadminClass;
                    ...
                    // Add announcement type indicator
                    const announcementType = announcement.announcement_type === 'superadmin' ? 'Superadmin' : 'Regular';
                    const typeIcon = announcement.announcement_type === 'superadmin' ? '👑' : '📢';
                    const badgeClass = announcement.announcement_type === 'superadmin' ? 'superadmin-badge' : 'badge bg-info';
                    const typeBadge = `<span class="${badgeClass} me-2">${typeIcon} ${announcementType}</span>`;
                    
                    item.innerHTML = `
                        <div class="d-flex w-100 justify-content-between">
                            <h5 class="mb-1">${typeBadge}${announcement.title}</h5>
                    ...
                    item.addEventListener('click', () => {
                        ...
                        const modalTitle = announcement.announcement_type === 'superadmin' ? '👑 Superadmin Announcement' : '📢 View Announcement';
                        Swal.fire({
                            title: modalTitle,
            ...
            function showAnnouncementModal(announcement) {
                ...
                // Add announcement type to title
                const announcementType = announcement.announcement_type === 'superadmin' ? '👑 Superadmin' : '📢 Regular';
                announcementTitle.innerHTML = `${announcement.title} <small class="text-muted">(${announcementType})</small>`;
